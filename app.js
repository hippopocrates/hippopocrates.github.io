$(() => {
  ...
  class Player {
    ...
    checkLying(currentPlayer) {
      ...
      if (
        currentPlayer.currentRoundDiscard + this.numCurrentPositionCards >
        4
      ) {
        bullshitCalled = true;
        $("#text-container").append(
          this.name + " accuses " + currentPlayer.name + " of lying!<br />"
        );
  ...
  const insertCardImgUser = () => {
    ...
    for (let i = 0; i < user.deck.length; i++) {
      $playerCardImg = $("<img>")
        .attr({ src: user.deck[i].image, id: user.deck[i].code })
        .addClass(user.deck[i].value);
      ...
      $playerCardImg.on("click", event => {
        if (user.turn) {
          ...
          $("#card-container").append(
            $("<img>").attr("src", "playing-card-back.jpg")
          );
  ...
  const alertMove = player => {
    if (player.currentRoundDiscard === 0) {
      $("#text-container").append(
        player.name + " played NO " + cardOrderPosition() + "'s! <br />"
      );
  ...
  $("#bs-btn").on("click", () => {
    if (bullshitCalled === false && user.turn === false && !winner()) {
      $("#text-container").append(
        user.name +
          " accused " +
          returnCurrentPlayer().name +
          " of lying!<br />"
      );
      if (returnCurrentPlayer().isLying) {
        $("#text-container").append(
          returnCurrentPlayer().name + " WAS lying!<br />"
        );
        returnCurrentPlayer().deck = returnCurrentPlayer().deck.concat(
          discardPile
        );
        ...
        $("#text-container").append(
          returnCurrentPlayer().name + " was NOT lying!<br />"
        );
  ...
  $("#instructions-btn").on("click", () => {
    $("#modal-subtext").html(
      "<p>After you've completed your turn, click the 'done' button. Whenever an opponent plays, either choose 'bullshit' if you think they are lying, or 'checks out' if you think they're good people.</p><p>You go first, my girl.</p>"
    );
  });
